// features/eval/componentsEval/Evalsurveymodal.tsx
...
interface Props {
  ...
  callReasons: { id: number; libelle: string }[]
}
...
const S = {
  ...
  sectionTitle: {
    ...
    borderBottom: '2px solid #DC2626', paddingBottom: 4,
...
const SurveyEditModal: React.FC<Props> = ({
  ...
  // ✅ FIX : items transmis avec `id` — le remapping templateItemId se fait dans EvalPage
  const handleSubmit = () => {
    ...
    <div style={modalOverlay}>
      <div style={{
        ...
        <div style={modalHeader}>
          ...
          <span style={modalHeaderTitle}>Évaluation — {surveyLabel}</span>
                  ...
                  <div key={it.id}>
                    <div style={S.itemRow}>
                      <div style={S.questionTxt}>
                        <div>{it.question}</div>
                        {it.description && (
                          <div style={S.descTxt}>{it.description}</div>
                        )}
                      </div>
        ...
        {/* Classification — en dehors du scroll, au-dessus du footer */}
        <div style={S.sectionTitle}>Classification</div>
        <div style={S.classificationBlock}>
          <div style={S.selectRow}>
            ...
            <div>
              <label style={S.label}>Raison d'appel</label>
              <select
                style={S.select}
                value={callReasonId}
                onChange={e => setCallReasonId(e.target.value)}
              >
                <option value="">— Sélectionner —</option>
                {callReasons.map(c => (
                  <option key={c.id} value={String(c.id)}>{c.libelle}</option>
          ...
          <div style={S.formGroup}>
            <label style={S.label}>CC Email (optionnel)</label>
            <textarea
              style={{ ...S.textarea, minHeight: 36 }}
              placeholder="superviseur@company.com"
              rows={1}
              value={ccEmail}
